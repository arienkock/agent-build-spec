...
| `resume.py` | Consistency → ERROR before discrepancy; all skipped → COMPLETE; all completed → COMPLETE; gap → ERROR; running at last → NEEDS_CONFIRMATION; running not at last → ERROR; failed → READY; completed with remaining → READY (next); `last_task` is skipped → READY (next) or COMPLETE |
| `preflight.py` | `O_EXCL` atomicity; stale PID with mismatched cmdline → refuse; absent PID → acquire; `src/` absent → error; corrupted/non-parseable lock file → refuse with error message naming lock file path; **empty lock file content → refuse (treated as unverifiable, same as corrupted)**; tmp `.tmp` files deleted before dirty-tree check (not after) |
...
| `task_run.py` | `max_retries` exhausted → failed + non-zero exit; lock released on exception; global prompt conditional on GLOBAL.md actually copied; retry prompt format (latest failure only); agent non-zero → verification skipped; `.agent-context/` removed on success only; commit aborts cleanly if no changes; timeout retry uses original unchanged prompt; explicit targeting with failed intermediate → abort before confirmation; **explicit targeting with non-existent task ID → abort before any side effects**; `base_commit` from prior running record preserved into new running record (read before overwrite, not after); **shared `max_retries` counter: one timeout retry + one verification failure retry together exhaust `max_retries=2` → failed** |
| `config.py` | Missing config file → all defaults applied; `agent_command` default contains `{model}` placeholder; negative/zero `agent_timeout_seconds` → validation error; missing individual fields → per-field defaults; extra unknown fields ignored |
...
| `rollback.py` | Missing `previousResults` file → abort before changes; null chain → delete only; skipped → abort; missing base commit → abort; guard order enforced; archive file is moved not copied — original archive absent after successful rollback; **no-op rollback (src/ already at base commit, no staged changes) → clear error, no empty commit** |
