...
| `resume.py` | Discrepancy check first (unknown task ID in latest AND archived → ERROR); consistency check (archived without latest → ERROR); no records → READY; all completed → COMPLETE; all skipped → COMPLETE; mixed completed/skipped sequence before last_task → valid (READY or COMPLETE); gap → ERROR; running at last → NEEDS_CONFIRMATION; running not at last → ERROR; failed → READY |
...
| `agent.py` | Prompt via stdin not argv; `cwd=src/`; SIGINT kills then `wait()` reaps; SIGTERM handler installed → kills subprocess + `wait()` reaps + re-raises (HIGH: verify `wait()` called before re-raise); timeout → kill + `wait()` + event; resume preserves `base_commit`; `OSError` → `failed`, no retry |
...
- **SIGTERM during agent (HIGH):** same outcome as SIGINT — subprocess killed, `process.wait()` reaps, lock released, `running` record persists → NEEDS_CONFIRMATION on next run
